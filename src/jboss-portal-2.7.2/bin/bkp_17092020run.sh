...
if [ "x$JAVA" = "x" ]; then
	...
	#JAVA="/server/jdk1.6.0_06/bin/java"
	JAVA="/server/jdk1.6.0_45/bin/java"
    else
	#JAVA="/server/jdk1.6.0_06/bin/java"
	JAVA="/server/jdk1.6.0_45/bin/java"
    fi
...
### Argumentos AppDynamics
export JAVA_OPTS="$JAVA_OPTS -javaagent:/opt/appdynamics/appserveragent/javaagent.jar -Dappdynamics.agent.applicationName=TCRAIA -Dappdynamics.agent.tierName=JBOSS -Dappdynamics.agent.nodeName=10.1.45.25 -Dappdynamics.analytics.agent.url=http://rdapm-analytics.raiadrogasil.com.br:80/v2/sinks/bt"
